...
class DMVST(nn.Module):
    def __init__(self, demand_embedding_dim, temporal_embedding_dim, context_embedding_dim, num_temporal_features, Local_cnn, LSTM, Line, grid_size, loss_fn=None):
        ...
        self.final_fc = nn.Linear(LSTM['hidden_size'] + context_embedding_dim, 1)
    ...
    def predict(self, demands, temporal_features=None, node_ids=None):
        ...
        if temporal_features is not None:
            ...
            temporal_emb = torch.zeros(B, T, self.temporal_embedding_dim).to(demands.device)

        if node_ids is not None:
            # print(f'type(node_ids): {type(node_ids)}, shape: {node_ids.shape}')
            context_emb = self.context_embedding_layer(self.line_embeddings[node_ids].to(demands.device)) #(B, D3)
        else:
            context_emb = torch.zeros(B, self.context_embedding_dim).to(demands.device)

        lstm_input = torch.cat([demands_features, temporal_emb], dim=-1) #(B, T, D1 + D2)
        lstm_out, _ = self.lstm(lstm_input) #(B, T, hidden_size)

        final_features = torch.cat([lstm_out[:, -1, :], context_emb], dim=-1) #(B, hidden_size + D3)
        output = self.final_fc(final_features) #(B, 1)
        return output.squeeze(-1)
    
    def forward(self, demands, temporal_features=None, node_ids=None, labels=None):
